...
protected function attributes($attributes, $isMultiple, $valueAttribute = true) {
  $tmp = array();
  $valueAttribute);
  // Если данный дескриптор может содержать атрибут value, // а его имени соответствует элемент в массиве значений, // то установить этот атрибут
  if ($valueAttribute && isset($attributes['name'])&& array_key_exists($attributes['name'],$this->values)) {    $attributes['value'] =$this->values[$attributes['name']];
}

foreach ($attributes as $k => $v) {
// Истинное логическое значение означает // логический атрибут
if (is_bool($v)) {
if ($v) { $tmp[] = $this->encode($k); } }
// иначе k = v else {
$value = $this->encode($v);
// Если это многозначный элемент, присоединить // квадратные скобки ([]) к его имени
...
protected function isOptionSelected($name, $value) { // Если для аргумента $name в массиве отсутствует // элемент, значит, этот элемент нельзя выбрать if (! isset($this->values[$name])) {
return false; }
// Если же для аргумента $name в массиве имеется
// элемент, который сам является массивом, проверить, // находится значение аргумента $value в массиве
else if (is_array($this->values[$name])) {

 }
return in_array($value, $this->values[$name]); }
// А иначе сравнить значение аргумента $value с
// элементом массива значений по значению аргумента $name else {
return $value == $this->values[$name]; }
public function encode($s) { return htmlentities($s);
} }
 ?>
